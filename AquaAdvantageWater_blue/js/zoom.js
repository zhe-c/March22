(function($) {
	...
	function openPrevious() {
		var prev = openedImage.parent('.swiper-slide').prev();
		if (prev.length == 0)
			prev = $('.swiper-wrapper .swiper-slide:last-child');
		prev.find('a').trigger('click');
	}
	
	function openNext() {
		var next = openedImage.parent('.swiper-slide').next();
		if (next.length == 0)
			next = $('.swiper-wrapper .swiper-slide:first-child');
	...
	(function bindNavigation() {
		...
		if ($('.swiper-wrapper .swiper-slide a').length == 1)
			$('.swiper-wrapper .swiper-slide a')[0].addClass('zoom');
		$('.zoom, .swiper-wrapper .swiper-slide a').on('click', open);
	})();
